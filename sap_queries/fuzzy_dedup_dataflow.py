...
def transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Uses a single-function architecture and pure standard Python loops
    to absolutely guarantee it will not trigger hidden Pandas multi-threading exceptions.
    """
    if df.empty:
        return df
        
    # 1. Convert to pure Python list of dicts to bypass Pandas C-level threading
    records = df.to_dict('records')
    
    # 2. Pure Python Data Prep & Blocking
    blocks = {}
    for r in records:
        kunnr = str(r.get('KUNNR', ''))
        land1 = str(r.get('LAND1', ''))
        
        # Build text string to compare (excluding KUNNR and LAND1)
    ...
    # 3. Simple Nested Loop for Candidate Pairs (Bypassing pd.merge)
    for land1, block_records in blocks.items():
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                # INLINE LEVENSHTEIN ALGORITHM (Single thread loop)
                s1, s2 = t1, t2
                if len(s1) < len(s2):
                    s1, s2 = s2, s1
    ...
    # 5. Output Extraction & Filtering (Bypassing Pandas GroupBy)
    output_rows = []
