...
def get_consistent_empty_df(df: pd.DataFrame) -> pd.DataFrame:
    text_cols = [c for c in df.columns if c not in ('KUNNR', 'LAND1')]
    output_cols = ['KUNNR', 'Cluster_ID', 'Fuzzy_Score', 'CommonPart', 'UncommonPart', 'null_cnt']
    for c in text_cols:
        if c not in ('KUNNR', 'NAME2'):
            output_cols.append(f'{c}_null_flag')
            
    dummy_row = {col: None for col in output_cols}
    dummy_row['KUNNR'] = 'NO_DATA'
    dummy_row['Cluster_ID'] = 'NO_DATA'
    dummy_row['Fuzzy_Score'] = 'N/A'
    dummy_row['CommonPart'] = 'No near duplicates found or empty dataset provided.'
    dummy_row['UncommonPart'] = 'N/A'
    dummy_row['null_cnt'] = 0
    for c in text_cols:
        if c not in ('KUNNR', 'NAME2'):
            dummy_row[f'{c}_null_flag'] = False
            
    return pd.DataFrame([dummy_row], columns=output_cols)
...
def transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        if df.empty:
            return get_consistent_empty_df(df)
        ...
        try:
            ...
            # Guarantee exactly the same column schema and order
            output_cols = ['KUNNR', 'Cluster_ID', 'Fuzzy_Score', 'CommonPart', 'UncommonPart', 'null_cnt']
            for c in text_cols:
                if c not in ('KUNNR', 'NAME2'):
                    output_cols.append(f'{c}_null_flag')
