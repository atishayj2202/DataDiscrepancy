...
def render_data_entry_inspector():
    if not st.session_state.audit_run:
        ...
        if not data_entry_findings:
            ...
            # Dropdown selection for Column Name
            selected_col = st.selectbox(
                "Select Column Name with Data Entry Errors:",
                list(grouped_findings.keys()),
                key="data_entry_col_select"
            )
            
            col_findings = grouped_findings[selected_col]
            
            # Rank all issues based of criticality (High > Medium > Low)
            ...
            st.markdown(f"### 📋 Audit Findings for Column `{selected_col}`")
            ...
            for idx, finding in enumerate(col_findings):
                ...
                # Show basic details separated by "|" & ":" (no verbose text below issue)
                ...
                highlighted_header = f"⚠️ {selected_col}"
                display_df = affected_df.rename(columns={selected_col: highlighted_header})
